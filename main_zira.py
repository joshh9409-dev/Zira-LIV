...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are Zira, Josh's personal AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "Zira checks these topics once a day and alerts the user when there is a new development. "
    ...
    },
    {
        ...
        "description": (
            ...
            "close the assistant, say goodbye, or stop Zira. "
...
class JarvisLive:
    def __init__(self, ui: JarvisUI):
        ...
        if get_push_to_talk_enabled():
            try:
                ...
                print(f"[Zira] ⚠ Push-to-talk unavailable: {e}")
    ...
    def _on_wake_detected(self) -> None:
        """Called from the detector thread when the configured Zira wake word is heard."""
    ...
    def sleep(self, reason: str = "timeout") -> None:
        ...
        self.ui.write_log(f"SYS: Sleeping — {reason}. Say 'Zira, online' to wake me.")
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask Zira to
    ...
    def _on_text_command(self, text: str):
        ...
        # "Zira, online" or the WAKE NOW button.
        if self._wake_enabled and not self._awake:
            self.ui.write_log("SYS: I'm asleep — say 'Zira, online' or tap WAKE NOW first.")
            return
    ...
    def interrupt(self) -> None:
        """Stop Zira mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[Zira] ✋ Interrupted — {drained} audio chunks discarded")
        self.set_speaking(False)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "Zira").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "Zira"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — Zira can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Proactive audio: Zira stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        print(f"[Zira] 📤 {name} → {str(result)[:80]}")
    ...
    async def _listen_audio(self):
        print("[Zira] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            # ── Wake-word gate ───────────────────────────────────────────────
            # While asleep, the mic audio NEVER goes to Gemini (nothing is
            # streamed, so Zira can't respond to speech not addressed to it and
            ...
            # ── Barge-in ─────────────────────────────────────────────────────
            # While Zira talks the mic is not streamed, but it is still worth
            # listening to locally: if the user starts speaking, cut the answer
            # short the way a person would stop when interrupted.
            #
            # The whole difficulty is echo — on speakers the mic hears Zira.
            ...
            if jarvis_speaking:
                # Nothing is streamed while Zira talks.
                #
        ...
        try:
            ...
            if _mic_dev is not None:
                print(f"[Zira] 🎤 Input device: {_mic_name}")
            try:
                ...
                if _mic_dev is None:
                    raise
                print(f"[Zira] ⚠️  Mic '{_mic_name}' failed: {_e} — using default")
                self.ui.write_log(
    ...
    async def _flush_pending_vision(self) -> bool:
        ...
        if self._vision_cam_active:
            # Camera: stay busy until Zira has finished speaking the answer,
                    ...
                    if _sru is not None:
                        if getattr(_sru, "resumable", False) and getattr(_sru, "new_handle", None):
                            if self._resume_handle is None:
                                print("[Zira] 🔗 Session resumption armed")
    ...
    async def _play_audio(self):
        ...
        try:
            ...
            if _spk_dev is None:
                raise
            print(f"[Zira] ⚠️  Output device '{_spk_name}' failed: {_e} — using default")
        ...
        try:
            ...
            print(f"[Zira] 🔊 Output latency {self._out_latency*1000:.0f} ms "
        ...
        try:
            while True:
                ...
                # Drive the HUD waveform and the avatar's mouth from Zira's
    ...
    async def _send_startup_briefing(self) -> None:
        ...
        print("[Zira] Briefing phase 1 (greeting) sent.")
        ...
        async def _deliver_news():
            try:
                ...
                print("[Zira] Briefing phase 2 (news) sent.")
            except Exception as e:
                print(f"[Briefing] Phase 2 error: {e}")
                print(f"[Zira] Briefing phase 2 failed: {e}")
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session and self._awake:
                # Don't interrupt if user spoke recently or Zira is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                if self.session:
                    ...
                    # has no desktop WAKE button — so it wakes Zira if asleep.
                    if self._wake_enabled and not self._awake:
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # until the user says "Zira, online" or taps wake in the UI.
                    if self._wake_enabled:
                        self._ensure_wake_detector()
                        self._awake = False
                        self.ui.set_state("SLEEPING")
                        self.ui.write_log("SYS: Zira online — sleeping. Say 'Zira, online' to wake me.")
                    else:
                        self._awake = True
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: Zira online.")
                ...
                if _is_reconnect_signal(e):
                    print("[Zira] Voluntary reconnect requested.")
                ...
                if _resumed_with and (
                    ...
                    print("[Zira] 🔗 Resumption handle rejected — starting a fresh session")
                ...
                err_str = str(e)
                print(f"[Zira] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if self._tuned_live and (
                    ...
                    print("[Zira] Live tuning rejected — reconnecting without it.")
                    continue
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[Zira] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[Zira] Reconnecting in {delay}s...")
